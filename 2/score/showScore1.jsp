...
.eXtremeTable .tableTotalEmpty {
	...
	<body style="margin: 25px;" bgcolor=#39867b>
			...
			<table width="100%" height="20" border="0" cellpadding="0"
				...
				<tr>
					<td height="20" class="title_s">
						<font color="white"><b>课程审核管理</b> </font><font color=pink size=2>(说明：管理员用户能审核所有学生选课，教师用户只能审核自己带的课，学生用户可以查看自己选课情况)</font>
					</td>
				</tr>
				<tr>
		...
		<ec:table items="scorelist"
			action="${pageContext.request.contextPath}/score/list.do"
			imagePath="${pageContext.request.contextPath}/images/table/*.gif"   
			
			title="提示您：输入框输入数据可进行搜索查询" rowsDisplayed="10" var="student">
			...
			<ec:row>
				...
				<ec:column sortable="false" filterable="false" viewsDenied="xls"
					alias="id2" title="教师审核" >
					<html:link action="/score/shenhe?stuId=${student.student.id}&courseId=${student.course.id}&state=${student.state}"
						onclick="return confirm('是否改变学生选修该门课程的状态？')">
							<c:if test="${student.state==1}">
							    <font style="color: red;">已审核</font>
							 </c:if>
							<c:if test="${student.state==-1}"><font color="blue">待审核</font></c:if>
