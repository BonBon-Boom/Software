...
.eXtremeTable .tableTotalEmpty {
	...
	<body style="margin: 25px;" bgcolor=#39867b> 
		 <td width="910"><table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td height="20" class="title_s"><font color="white"><b>公告管理</b></font></td>
          </tr>
          <tr>
		...
		<ec:table items="noticelist"
			...
			title="提示您：输入框输入数据可进行搜索查询,更新公告后请及时刷新页面" width="100%" rowsDisplayed="5" var="notice">
			<ec:exportXls fileName="notice.xls" tooltip="Excel" />
			<ec:row>
				...
				<ec:column sortable="false" filterable="false" viewsDenied="xls"
					alias="id1" title="编辑">
					<html:link action="/notice/toEdit?id=${notice.id}" onclick="return confirm('确定更新公告？')"><font color='blue'>编辑</font></html:link>
				</ec:column>
				<ec:column sortable="false" filterable="false" viewsDenied="xls"
					alias="id1" title="删除">
					<html:link action="/notice/delete?id=${notice.id}"
						onclick="return confirm('确定删除公告？')"> <font color='blue'>删除</font> </html:link>
				</ec:column>
			</ec:row>
		</ec:table>
