...
.eXtremeTable .tableTotalEmpty {
	...
	<body style="margin: 25px;" bgcolor=#39867b>
		...
		<td width="910"><table width="100%" height="20"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td height="20" class="title_s"><font color="white"><b>班级管理</b></font></td>
          </tr>
          <tr>
		...
		<br>
		<ec:table items="classeslist"
			action="${pageContext.request.contextPath}/classes/list.do"
			imagePath="${pageContext.request.contextPath}/images/table/*.gif"
			title="提示您：输入框输入数据可进行搜索查询" width="96%" rowsDisplayed="10" var="classes">
			<ec:exportXls fileName="classes.xls" tooltip="Excel" />
			<ec:row>
				<ec:column property="classesNo" title="班级编号" /> 
                <ec:column property="professions.name" title="所属专业"/> 
                <ec:column property="professions.collage.name" title="所属学院"/> 
			</ec:row>
			
		 
				<ec:column sortable="false" filterable="false" viewsDenied="xls"
					alias="id1" title="删除">
					<html:link action="/classes/delete?id=${classes.id}"
						onclick="return confirm('确定删除专业信息？')"> 
	              <%
